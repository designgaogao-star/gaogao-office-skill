...
- Treat first-assignment role calibration as part of the first real dispatch, not employee startup. Before the first real task for an employee, let the user choose light, standard, deep, or skipped calibration; the employee writes project-specific role judgment into its own `memory.md`.
- Prefer file-first inter-agent communication. Full dispatch packets and full employee reports should live in office files when practical; thread messages should carry task title, file path, status, and whether user input is needed.
...
10. When employees are available, operate through controller-dispatch by default:
   ...
   - before an employee's first real assignment, ask the user to choose a role-calibration level: A light, B standard, C deep, or D skip. Deep calibration requires separate approval for broader reading, web research, or external references
   ...
   - use file-first dispatch when practical: write the full handoff to an office file and send only the task title, file path, state, and return requirement through the employee thread
...
Employee report return is also a thread action, but the default is file-first. When practical, employees write the full report to an office file such as `Agent Office/Exchange/Reports/{task-title}.md`, then use `send_message_to_thread` only to send the report path, status, and whether user input is needed. If the project-director thread ID is `current-window`, `TBD`, missing, stale, or uncertain, if thread tools are unavailable, or if file writing is unavailable, the employee must output a copyable report in its own chat and clearly say it needs to be copied back to the project-director chat.
