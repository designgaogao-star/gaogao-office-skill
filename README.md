...
<details>
<summary><strong>简体中文说明（点击展开）</strong></summary>
...
v1.0.4 修正文件优先通信协议：员工把项目语境沉淀成自己的岗位判断标准，完整派工包和完整汇报优先写入 `Agent Office/Exchange/`，线程里只传任务名、路径、状态和是否需要用户介入。
...
v1.0.4 fixes the file-first employee communication protocol. Employees turn project context into role-specific judgment in their own memory, while full dispatch packets and full reports live in `Agent Office/Exchange/` when practical; thread messages carry task title, paths, status, and user-input needs.
